...
const ALL_PAGES = [
  ...
  {
    to: "/bar", label: "Bar 🍺", roles: ["root", "admin", "gerente", "garcon"],
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function Navbar() {
  ...
  const isDevAllowed = (user?.role === "admin" || user?.role === "root")
    && (import.meta.env.DEV || import.meta.env.VITE_DEVTOOLS_ENABLED === "true");
  ...
  return (
    <>
        ...
        {user && (
          <div className="flex items-center gap-2 flex-shrink-0 pl-2" style={{ borderLeft: "1px solid rgba(255,255,255,0.18)" }}>
            ...
            <span
              ...
              style={{ background: isPrivileged ? O : "rgba(255,255,255,0.15)", color: W }}
            >
              {ROLE_LABELS[user?.role] ?? user?.role ?? "—"}
            </span>

            {/* Botão Dev Tools — visível apenas para admin/root em DEV */}
            {isDevAllowed && (
              <button
                ...
                style={{
                  ...
                  background: devOpen ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.10)",
                  border: `1px solid ${devOpen ? "rgba(255,255,255,0.4)" : "rgba(255,255,255,0.2)"}`,
                  color: W, cursor: "pointer",
                }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(255,255,255,0.22)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = devOpen ? "rgba(255,255,255,0.22)" : "rgba(255,255,255,0.10)")}
              >
      ...
      {isDevAllowed && devOpen && (() => {
        const expiry  = token ? tokenExpiresIn(token) : "—";
        const expired = expiry === "Expirado";
        return (
          <div style={{
            position: "fixed", top: 58, right: 12, zIndex: 9998,
            background: "rgba(14,14,14,0.96)", border: "1px solid rgba(255,255,255,0.1)",
            borderRadius: 12, padding: "14px 16px", minWidth: 230,
            backdropFilter: "blur(10px)", boxShadow: "0 8px 32px rgba(0,0,0,0.5)",
          }}>
            <p style={{ fontSize: 9, fontWeight: 800, color: "rgba(255,255,255,0.3)",
              letterSpacing: "0.12em", marginBottom: 12, textTransform: "uppercase" }}>
              Dev Tools
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {[
                { label: "Usuário",        value: user?.username,                  color: "rgba(255,255,255,0.85)" },
                { label: "Role",           value: user?.role?.toUpperCase(),       color: user?.role === "root" ? "#ef4444" : "#3b82f6" },
                { label: "Token expira em", value: expiry,                         color: expired ? "#ef4444" : "#4ade80" },
