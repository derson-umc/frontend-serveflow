...
export default function TabPayables() {
  ...
  const { data: list = [], isLoading } = useQuery({
    queryKey: QK.payables,
    queryFn:  () => financialApi.payables.list(),
    staleTime: 60_000,
  });

  const createMutation = useMutation({
    mutationFn: (data) => financialApi.payables.create(data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: QK.payables });
  ...
  const settleMutation = useMutation({
    mutationFn: ({ id, amount }) => financialApi.payables.settle(id, { amount }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: QK.payables });
    ...
    onError: (e) => toast.error(e?.response?.data?.error ?? "Erro ao registrar pagamento."),
  });

  const cancelMutation = useMutation({
    mutationFn: (id) => financialApi.payables.cancel(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: QK.payables });
  ...
  const handleSettle = () => {
    if (!settleAmt) { toast.error("Informe o valor pago."); return; }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <button onClick={() => setShowCreate(true)} style={{ padding: "9px 18px", borderRadius: 10, background: palette.red, color: palette.white, border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer", boxShadow: "0 4px 12px rgba(198,40,40,0.22)" }}>
          + Nova Conta
        </button>
      </div>
              ...
              <AnimatePresence>
                {filtered.map((r, idx) => (
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.02 }}
                    ...
                    <td style={{ padding: "10px 14px", fontWeight: 700, color: palette.red }}>{fmtBRL(r.amount)}</td>
                    <td style={{ padding: "10px 14px" }}><StatusBadge status={r.status} /></td>
                    <td style={{ padding: "10px 14px", color: palette.textMuted }}>{r.supplier || "—"}</td>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Nova Conta a Pagar">
        <FormField label="Descrição" required>
          <input value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder="Ex: Compra de insumos" style={inputStyle} />
        </FormField>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <FormField label="Categoria">
            <input value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))} placeholder="Ex: INSUMO, ALUGUEL..." style={inputStyle} />
          </FormField>
          <FormField label="Fornecedor">
            <input value={form.supplier} onChange={(e) => setForm((p) => ({ ...p, supplier: e.target.value }))} placeholder="Nome do fornecedor" style={inputStyle} />
          </FormField>
        </div>
        <button onClick={handleCreate} disabled={createMutation.isPending}
          style={{ width: "100%", padding: "11px", borderRadius: 10, background: createMutation.isPending ? palette.border : palette.red, color: palette.white, border: "none", fontWeight: 700, fontSize: 14, cursor: createMutation.isPending ? "not-allowed" : "pointer" }}>
          {createMutation.isPending ? "Salvando..." : "Criar Conta a Pagar"}
        </button>
      </Modal>
      ...
      <Modal open={!!showSettle} onClose={() => setShowSettle(null)} title="Registrar Pagamento" subtitle={showSettle?.description}>
        ...
        <button onClick={handleSettle} disabled={settleMutation.isPending}
          ...
          {settleMutation.isPending ? "Registrando..." : "Confirmar Pagamento"}
        </button>
      </Modal>
    </div>
  );
